...
function handlePost() {
    ...
    const mainTextInput = document.getElementById('mainTextInput').value.trim();
    
    // 少なくとも1つの入力が必要
    if (!salesPageInput && !sampleInput && !mainTextInput) {
        alert('投稿内容を入力してください');
        return;
    }
    ...
    const newPost = {
        ...
        hashtags: extractHashtags(mainTextInput) // 本文からハッシュタグを抽出
    };
